...
def _extract_html(bs_data):
    ...
    for item in tqdm(k):
        ...
        postTimeStamp = item.find_all(class_="c_1lsm09qaw7 x_1lsm09kvz8 clearfix")[0]
        if  postTimeStamp.find(class_="_5ptz") is not None:
            time =  postTimeStamp.find(class_="_5ptz").get("title")
            postDict["timestamp"] = time
        ...
        for posts in actualPosts:
            ...
            postDict['Post'] = unidecode.unidecode(text)
        ...
        for comment in postComments:
            ...
            if comment_text is not None:
                postDict['Comments'][commenter]["text"] = unidecode.unidecode(comment_text.text) 
            ...
            if comment_pic is not None:
                if comment_pic.find(class_="img") is not None:
                    postDict['Comments'][commenter]["image"] = comment_pic.find(class_="img").get("src")
                    postDict['Comments'][commenter]["image_desc"] = comment_pic.find(class_="img").get("alt")

            comment_reaction = comment.find(class_="_6cuq")
            if comment_reaction is not None:
                reaction_count = comment_reaction.find("span", class_="_1lld")
                if reaction_count is not None:
                    postDict["Comments"][commenter]["reaction_count"] = reaction_count.text
            
            comment_time = comment.find(class_="_6qw7")
            if comment_time is not None:
                postDict['Comments'][commenter]["timestamp"] = comment_time.find(class_="livetimestamp").get("data-tooltip-content")
    ...
    if args.usage == "WT":
        with open('output.txt', 'w') as file:
            ...
            for post in postBigDict:
                _data["data"].append(post)  # use json load to recover
            #file.write(_data)
